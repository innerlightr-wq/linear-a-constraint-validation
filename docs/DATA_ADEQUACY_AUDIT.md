...
| §4 Rule A block boundary (ruling / prior-total) | **DERIVABLE, with a stated caveat** | `"\n"` used as the ruling proxy (N1); not independently confirmed identical to a physical incised ruling in the GORILA epigraphic sense — an honest AMBIGUITY, not a blocker, since the frozen protocol's `ruling` concept is satisfied by *a* documented boundary signal, and this is the best one this source offers |
